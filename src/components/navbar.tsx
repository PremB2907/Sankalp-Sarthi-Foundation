    ...
    <header className="sticky top-0 z-40 bg-[#F7F6F0]/95 backdrop-blur-md border-b border-[#17352D]/10 transition-colors w-full max-w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          {/* Mobile Right Controls: Modern Android 15 App Bar Button */}
          <div className="flex items-center gap-2 lg:hidden">
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="w-11 h-11 bg-[#003D31] text-white rounded-full flex items-center justify-center shadow-md active:scale-95 transition-transform focus:outline-none"
              aria-expanded={mobileMenuOpen}
              aria-label="Toggle Mobile App Navigation Menu"
            >
      ...
      {/* Modern Android 15+ / iOS Flagship Navigation App Sheet */}
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 top-16 z-50 bg-[#002D24] text-white flex flex-col justify-between p-5 sm:p-6 overflow-y-auto animate-in slide-in-from-top-4 duration-300 w-full max-w-full">
          
          <div className="space-y-6">
            
            {/* Top Quick App Actions */}
            <div className="grid grid-cols-2 gap-3">
              <Link
                href="/donate"
                onClick={() => setMobileMenuOpen(false)}
                className="p-4 bg-[#63BE21] text-[#003D31] rounded-2xl flex flex-col justify-between space-y-3 font-sans font-bold shadow-lg active:scale-98 transition-transform"
              >
                ...
                <div className="text-left">
                  <span className="text-[10px] font-extrabold uppercase tracking-widest block text-[#003D31]/70">Support Cause</span>
              ...
              <Link
                href="/volunteer"
                onClick={() => setMobileMenuOpen(false)}
                className="p-4 bg-emerald-950/90 border border-white/20 text-white rounded-2xl flex flex-col justify-between space-y-3 font-sans font-bold shadow-lg active:scale-98 transition-transform"
              >
                ...
                <div className="text-left">
                  <span className="text-[10px] font-extrabold uppercase tracking-widest block text-[#63BE21]">Join Field Team</span>
              ...
              <div className="space-y-1.5">
                {mainLinks.map((link) => (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="p-3.5 bg-emerald-950/40 hover:bg-emerald-950 border border-white/10 rounded-xl flex items-center justify-between group transition-all active:scale-98"
                  >
                    <div className="flex flex-col text-left">
                      <span className="font-serif font-bold text-lg text-white group-hover:text-[#63BE21] transition-colors leading-snug">
                        {link.name}
                      </span>
              ...
              <div className="grid grid-cols-2 gap-2">
                {secondaryLinks.map((link) => (
                  ...
                  >
                    ...
                    <span className="font-serif text-sm font-bold text-white leading-snug">
                      {link.name}
                    </span>
                  </Link>
                ))}
          ...
          {/* Bottom App Bar: WhatsApp Community & Social Channels */}
